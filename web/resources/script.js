...
function searchProducts(){
    ...
    jsonDataP.name = $('#name').val();
    jsonDataP.description = $('#description').val();
    jsonDataP.amountFromId = $('#amountFromId').val();
    jsonDataP.amountToId = $('#amountToId').val();
    jsonDataP.priceFromId = $('#priceFromId').val();
    jsonDataP.priceToId = $('#priceToId').val();

    let jsonStrP = JSON.stringify(jsonDataP);
    $.ajax({
        url: "http://localhost:8080/Store/search",
        type: 'POST',
        data: jsonStrP,
        dataType: 'json',
        async: true,
